...
class TestsSetActual:
    ...
    @pytest.mark.lp
    @pytestrail.case("C19035510", "C19035509", "C19034911", "C19011411")
    @pytest.mark.parametrize("doc_api, doc_ver_api, file_api",
                             DocumentVersionSetActual.DATA_FOR_CHANGING_VERSION_OF_DOCUMENT_ACTUAL)
    def test_make_first_version_of_document_actual(self, app, auth, doc_api, doc_ver_api, file_api):
        """Тест установки актуальности первой версии документа по ее ID."""

        # Создание документа и его версии.
        create_doc = app.doc.save_document(data=Document.for_saving_document_with_date(), url_api=doc_api)
        ...
        app.doc_ver_set_actual.set_document_version_actual(url_api=file_api, ver_id=create_ver['result']['id'],
                                                           set_actual=True)
        ...
        app.doc_ver_set_actual.set_document_version_actual(url_api=file_api, set_actual=True,
